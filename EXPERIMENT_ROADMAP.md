...
- **노이즈 유형**: 완벽한 표, 오탈자 포함, 누락된 값, 불완전한 구조
- **평가**: 노이즈 수준별 성능 저하 정도

### 실험 6: 임베딩 모델 비교
- **목적**: 한국어 특화 모델이 성능에 미치는 영향 분석
- **모델 비교**: 다국어 모델, 한국어 특화 모델, 최신 LLM 임베딩
- **평가**: 임베딩 품질 및 검색 성능

### 실험 7: 질의 유형별 성능 분석
- **목적**: 질의의 복잡도가 RAG 성능에 미치는 영향 분석
- **질의 유형**: 단순 조회, 집계, 비교, 추론 질의
- **평가**: 질의 유형별 정확도 및 응답 시간

### 실험 8: 파싱 전처리 기법 비교
- **목적**: 전처리 방법이 파싱 정확도에 미치는 영향 분석
- **전처리 기법**: OCR 후처리, 직접 파싱, 표 구조 인식
...
- **TEDS**: Tree Edit Distance-based Similarity 논문
- **GriTS**: Grid Table Similarity 논문
